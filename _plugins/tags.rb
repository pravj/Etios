...
module Jekyll
      ...
      site.posts.each do |post|
        ...
        post.tags.each do |tag|
          ...
          if tagdata.has_key?(tag)
            tagdata[tag]["content"] << {"url"=>post.url, "title"=>post.title}
          else
            tagdata[tag] = {"color"=>(Digest::SHA1.hexdigest(tag))[20..25], "content"=>[{"url"=>post.url, "title"=>post.title}]}
          end
        end
      end
  ...
  class TagPage < Page
    def initialize(site, base, dir, name, tagdata)
      ...
      self.data['tagdata'] = Hash[tagdata.sort_by{ |k, v| k.downcase }]
    end
  end
end
